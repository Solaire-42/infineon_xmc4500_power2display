...
void ssd1306_Reset(void) {
    // Reset the OLED
	XMC_GPIO_SetOutputLevel(SSD1306.rst_port, SSD1306.rst_pin, XMC_GPIO_OUTPUT_LEVEL_LOW);
	SSD1306.delay_fnc(10);
	XMC_GPIO_SetOutputLevel(SSD1306.rst_port, SSD1306.rst_pin, XMC_GPIO_OUTPUT_LEVEL_HIGH);
	SSD1306.delay_fnc(10);
}
...
void ssd1306_WriteCommand(uint8_t byte) {
	...
	XMC_GPIO_SetOutputLevel(SSD1306.dc_port, SSD1306.dc_pin, XMC_GPIO_OUTPUT_LEVEL_LOW);

	while (XMC_USIC_CH_TXFIFO_IsFull(SSD1306.channel)) __NOP();
	XMC_SPI_CH_Transmit(SSD1306.channel, byte, XMC_SPI_CH_MODE_STANDARD);

	while (fallingEdge == 0)
	{
		XMC_SPI_CH_ClearStatusFlag(SSD1306.channel, XMC_SPI_CH_STATUS_FLAG_MSLS_EVENT_DETECTED);
		while (!(XMC_SPI_CH_GetStatusFlag(SSD1306.channel) & XMC_SPI_CH_STATUS_FLAG_MSLS_EVENT_DETECTED)) __NOP();
		if (XMC_SPI_CH_GetStatusFlag(SSD1306.channel) & XMC_SPI_CH_STATUS_FLAG_MSLS)
			fallingEdge = 0;
		else
			fallingEdge = 1;
	}
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
	...
	XMC_GPIO_SetOutputLevel(SSD1306.dc_port, SSD1306.dc_pin, XMC_GPIO_OUTPUT_LEVEL_HIGH);

	for (uint32_t i=0; i< buff_size; i++)
	{
		while (XMC_USIC_CH_TXFIFO_IsFull(SSD1306.channel)) __NOP();
		XMC_SPI_CH_Transmit(SSD1306.channel, buffer[i], XMC_SPI_CH_MODE_STANDARD);
	}
